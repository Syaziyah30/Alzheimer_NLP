...
# Load JSON and apply text cleaning
def load_json(directory):
    data = []
    for file in directory.glob("*.json"):
        with open(file, encoding="utf-8") as f:
            content = json.load(f)
            text = clean_text(content.get("content", ""))
            # Apply preprocessing here to handle <240 and other placeholders
...
def prepare_dataset(json_data, semantics):
    ...
    for item in json_data:
        ...
        data.append({
            "filename": item["filename"],  # ✅ add this line
            "tokens": tokens,
            "ner_tags": labels
        })
...
dataset, all_entities = prepare_dataset(json_data, semantics)
...
REPORT_FILE = BASE_DIR / "classification_report.json"
...
# Save label list, label2id, and id2label to OUTPUT_DIR
with open(OUTPUT_DIR / "label_list.json", "w") as f:
